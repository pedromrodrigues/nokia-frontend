...
export default class VrfsView extends Vue {
    ...
    async created() {
        try {
            ...
            this.vrfs = await RemoteServices.getVrfs(this.$store.getters.getCurrentSwitch.hostname, this.vrf_type);
        } catch (error) {
    ...
    async showConfig(vrf: Vrf) {
        try {
            this.config = await RemoteServices.getVrfConfig(this.$store.getters.getCurrentSwitch.hostname, vrf.name);
