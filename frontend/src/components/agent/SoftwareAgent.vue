  ...
  <div class="text-center">
    <v-container fluid>
      <v-form ref="form" v-model="valid" lazy-validation>
        ...
        <v-btn color="grey" class="mr-4" @click="reset"> Reset </v-btn>
        <v-btn color="blue" class="mr-4" @click="close">Cancel </v-btn>
        <v-btn :disabled="!valid" color="blue" class="mr-4" @click="runTest">
          Run
        </v-btn>
      </v-form>
    </v-container>
...
export default class SoftwareAgent extends Vue {
  ...
  async created() {
    try {
      ...
      this.ipVrfs = await RemoteServices.getVrfs(
  ...
  async runTest() {
    if ((this.$refs.form as HTMLFormElement).validate()) {
      ...
      try {
        await RemoteServices.setAgentRunning(
          this.$store.getters.getCurrentSwitch.hostname,
          formData
        );
        this.close();
        ...
        await this.$store.dispatch("error", "Validation failed");
    }
  }
